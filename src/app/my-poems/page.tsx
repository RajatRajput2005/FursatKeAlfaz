...
export default function MyPoemsPage() {
  ...
  useEffect(() => {
    const fetchPoems = async () => {
      try {
        ...
        const res = await axios.get("/api/mypoems", {
  ...
  try {
    ...
    await axios.delete("/api/poem", {
      headers: { Authorization: `Bearer ${token}` },
      data: { poemId }
    });

    alert("Deleted Successfully!");
    setPoems(prev => prev.filter((p: any) => p.id !== poemId));
  } catch (err: any) {
        ...
        >
          {poems.map((poem: any) => (
            ...
            >
              ...
              <div style={{ display: "flex", gap: "12px", alignSelf: "center", marginTop: "auto" }}>
                <button
                ...
                <Link href={`/edit-poem/${poem.id}`}>
                  <button
                    ...
                    onMouseEnter={e => (e.currentTarget.style.background = "linear-gradient(90deg, #dc2430 0%, #7b4397 100%)")}
                    onMouseLeave={e => (e.currentTarget.style.background = "linear-gradient(90deg, #7b4397 0%, #dc2430 100%)")}
                  >
                    ✏️ Edit
                  </button>
                </Link>
